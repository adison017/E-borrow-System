        ...
        case "rejected":
            return (
              <div className="inline-flex items-center gap-1 rounded-lg bg-red-100 px-3 py-1.5 text-red-700 text-sm font-semibold">
                <XCircleIcon className="w-5 h-5" /> ไม่ผ่านการตรวจสอบ
              </div>
            );
        default:
          return (
      ...
      <div className={`modal ${isOpen ? 'modal-open' : ''}`}>
        ...
        <div className="modal-box w-11/12 max-w-4xl p-0 bg-white rounded-lg shadow-2xl z-50 transform transition-all duration-300 ease-out scale-95 opacity-0 
          ...
          {borrow.status === "under_review" && (
            <div className="bg-gray-100 px-6 py-4 flex flex-col gap-4 border-t">
              {showRejectReason && (
              ...
              <div className="flex justify-end gap-2">
                ...
                {!showRejectReason ? (
                  ...
                  >
                    {isSubmitting ? 'กำลังประมวลผล...' : 'ยืนยันไม่ผ่านการตรวจสอบ'}
                  </button>
                )}
                ...
                >
                  {isSubmitting ? 'กำลังประมวลผล...' : 'ยืนยันข้อมูลและส่งอนุมัติ'}
                </button>
              </div>
            </div>
          )}
