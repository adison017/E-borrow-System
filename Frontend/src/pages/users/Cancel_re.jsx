...
const RequirementList = () => {
    ...
    fetch(`http://localhost:5000/api/borrows?user_id=${user_id}`)
      .then(async res => {
        ...
        try {
          ...
          // filter เฉพาะของ user_id และ status === 'rejected'
          if (Array.isArray(data)) {
            return data.filter(b => b.user_id == user_id && b.status === 'rejected');
          }
          return [];
        } catch {
          return [];
  ...
  // กรองเฉพาะ rejected
  const rejectedList = borrowList.filter(req => req.status === 'rejected');

  return (
    <div className="container mx-auto px-4 py-8">
      <h1 className="text-2xl font-bold text-gray-800 mb-6">รายการปฏิเสธอนุมัติการยืมครุภัณฑ์</h1>
      <div className="space-y-6">
        {rejectedList.length === 0 && (
          <div className="flex flex-col items-center justify-center py-12">
            ...
            <h3 className="text-xl font-bold text-yellow-600 mb-2">ไม่พบรายการที่ถูกปฏิเสธ</h3>
            <p className="text-gray-500 text-base">คุณไม่มีรายการที่ถูกปฏิเสธในขณะนี้</p>
          </div>
        )}
              ...
              <div className="flex flex-col md:flex-row">
                ...
                <div className="md:w-2/3 w-full">
                  <div className="card-body p-4 md:p-6">
                    ...
                    <div className="my-1 bg-red-50 p-3 rounded-lg">
                      <h3 className="font-semibold text-red-700 mb-1">เหตุผลการปฏิเสธ</h3>
                      <p className="text-red-800 text-xl font-bold">{request.rejection_reason && request.rejection_reason.trim() !== '' ? request.rejection_reason : '-'}</p>
                    </div>
                    <div className="mb-4">
