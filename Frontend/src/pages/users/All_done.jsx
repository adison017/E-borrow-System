...
const RequirementList = () => {
    ...
    fetch(`http://localhost:5000/api/borrows?user_id=${user_id}`)
      .then(async res => {
        ...
        try {
          ...
          // filter เฉพาะของ user_id และ status === 'completed'
          if (Array.isArray(data)) {
            return data.filter(b => b.user_id == user_id && b.status === 'completed');
          }
          return [];
        } catch {
          return [];
  ...
  // กรองเฉพาะ completed
  const completedList = borrowList.filter(req => req.status === 'completed');

  return (
    <div className="container mx-auto px-4 py-8">
      <h1 className="text-2xl font-bold text-gray-800 mb-6">รายการที่เสร็จสิ้น</h1>
      <div className="space-y-6">
        {completedList.length === 0 && (
          <div className="flex flex-col items-center justify-center py-12">
            ...
            <h3 className="text-xl font-bold text-yellow-600 mb-2">ไม่พบรายการที่เสร็จสิ้น</h3>
            <p className="text-gray-500 text-base">คุณไม่มีรายการที่เสร็จสิ้นในขณะนี้</p>
          </div>
        )}
        {completedList.map((request) => {
                ...
                <div className="md:w-2/3 w-full">
                  <div className="card-body p-4 md:p-6">
                    <div className="flex flex-col md:flex-row md:justify-between justify-center md:items-start items-center gap-2">
                      ...
                      <div className={`badge badge-neutral text-white md:text-base px-4 py-4 rounded-full text-sm font-medium`}>
                        เสร็จสิ้น
                      </div>
                    </div>
                    <div className="my-4">
